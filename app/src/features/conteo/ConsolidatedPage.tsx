...
import type { EstadoTriangulacion } from '../../domain/types';
...
export function ConsolidatedPage() {
  ...
  function exportar() {
    const csv = exportarConsolidadoCsv(filas);
    const blob = new Blob(['﻿' + csv], { type: 'text/csv;charset=utf-8' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `consolidado-${sesion!.nombre.replace(/\s+/g, '_')}.csv`;
    a.click();
    ...
    <>
      ...
      {!priv && (
        <p className="badge warn" style={{ display: 'block' }}>
          ...
          triangulación y el stock oficial los ve Admin / Auditor.
        </p>
      )}
      ...
      <div className="row" style={{ margin: '1rem 0' }}>
        ...
        <input
          ...
          style={{ maxWidth: 280 }}
        />
        <span className="muted">{filas.length} filas</span>
